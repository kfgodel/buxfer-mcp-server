...
- **Transaction** — **Done (2026-05-07).** Three-tier breakdown: 13 Tier 1 fixture-always-present required fields (`id`, `description`, `amount`, `date`, `tags`, `type`, `status`, `accountName`, `transactionType`, `expenseAmount`, `tagNames`, `isFutureDated`, `isPending`); 1 Tier 2 (`accountId: Int?` — key always present, value `null` on transfer records); 2 Tier 3 conditional-on-transfer (`fromAccount`, `toAccount`). `AccountRef.name` tightened to non-nullable. The `type` (string) + `transactionType` (string) duplication preserved per fixture rule (similar to Reminder). `BuxferClient.getTransactions()` returns `JsonObject` (the only list endpoint with a wrapper response shape: `transactions[]` + `numTransactions`); doesn't fit `getValidatedList` so kept inline. `TransactionsResult.numTransactions` declared `String` to match the wire (Buxfer quotes it). Old `getList<T>` helper deleted (zero callers). Parse-error test repointed to `/transaction_add` since the strict-decode-throw path now lives only in the write endpoints.
...
- **Reminder** — **Done (2026-05-07).** 15 fixture-always-present fields tightened to non-nullable (`id`, `name`, `description`, `startDate`, `periodUnit`, `periodSize`, `amount`, `accountId`, `nextExecution`, `dueDateDescription`, `numDaysForDueDate`, `tags`, `editMode`, `type`, `transactionType`); `stopDate: String?` (no default) for the always-present-always-null case; spec-only `period: String? = null` (the spec describes a singular `period` string while the fixture covers it via `periodUnit` + `periodSize`). The `type` (string) + `transactionType` (int) duplication is preserved per fixture rule. `BuxferClient.getReminders()` uses `getValidatedList<List<Reminder>>("/reminders")`. Parse-error test repointed from `/reminders` to `/groups`.
- **Group / Member** — **Done (2026-05-07).** Fixture is empty (`[]`), so no fields could be tightened with evidence. Schema is fully Tier 3: every field on both `Group` and `Member` lands as `Type? = null` (optional). The validator effectively no-ops on Group responses today; revisit when a real fixture is captured. `BuxferClient.getGroups()` uses `getValidatedList<List<Group>>("/groups")`. Parse-error test repointed from `/groups` to `/transactions` (the last unmigrated endpoint).
